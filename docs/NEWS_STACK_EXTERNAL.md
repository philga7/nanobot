...
- OSINT: handled by the built-in `osint` skill (queries 27 sources directly)
...
- `IngestSignalRequest`: normalized signal ingestion
...
A working **Qdrant**-first stack and optional **7/24 Office** builds live under:
...
**Secrets and env:** Compose only interpolates **ports** from the project `.env`. Per-service settings use optional `env_file` entries (Qdrant `.env.qdrant`), while 7/24 Office is mainly **`config.json`** in the vendor tree (often bind-mounted at runtime so keys are not only baked into the image). See [Configuration and secrets (Docker)](../deploy/news-stack/README.md#configuration-and-secrets-docker) in the deploy README. For a consolidated key list, see [News stack env reference](./NEWS_STACK_ENV_REFERENCE.md).

## Phased rollout: deploy, then Nanobot

Recommended order:

1. Bring up **Qdrant**, then **Office** using separate Compose profiles (`office` / `full`) — see [deploy/news-stack/README.md](../deploy/news-stack/README.md#phased-bring-up-one-service-at-a-time).
...
```text
OSINT skill -> nanobot agent -> 7/24 Office -> Qdrant
                                |
