...
def _run_gateway(
    ...
    *,
    ...
    agent = AgentLoop(
        ...
        memory_sqlite_path=(
            (
                Path(config.tools.memory.db_path).expanduser()
                if config.tools.memory.db_path
                else get_data_dir() / "memory.db"
            )
            if config.tools.memory.sqlite_enabled
            else None
        ),
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        """Execute a cron job through the agent, exec tool, or Dream."""
        import os
        ...
        if job.payload.kind == "shell_exec":
            ...
            if not isinstance(exec_tool, ExecTool):
                raise RuntimeError("shell_exec cron jobs require tools.exec enabled in config")
        ...
        async def _silent(*_args, **_kwargs):
            """Suppress progress streaming for cron-executed turns."""
            return None
        ...
        # Fallback ntfy only when deliver=true and the agent did not post (agent failure path).
        # When deliver=false, the agent owns delivery—including choosing not to notify—so never
        # treat "no channel post" as a reason to ping ntfy here.
        ...
        should_ntfy = job.payload.deliver and not agent_delivered
                ...
                try:
                    ...
                    if ntfy_resp.status_code >= 400:
                        logger.error(
                            ...
                            ntfy_resp.status_code,
                            job.id,
                            (ntfy_resp.text or "")[:200],
                        )
