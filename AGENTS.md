...
- **Skills** (`nanobot/skills/`): Built-in skill definitions (cron, github, image-generation, OSINT briefing, etc.) loaded into agent context. Sustained goals use `/goal` plus `long_task` / `goal_state` (no separate long-goal skill).
